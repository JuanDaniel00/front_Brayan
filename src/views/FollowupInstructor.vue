<template>
  ...
  <div id="container-buttons">
    <div class="searchButtons">
      <div class="InputButtonsSearch">
        ...
        />
        <buttonSearch
          :onclickButton="searchApprentice"
  ...
  <dialogSeeObservation
    v-model="isDialogVisibleObservation"
    title="OBSERVACIONES"
    labelClose="Cerrar"
    labelSend="Guardar"
    :onclickClose="closeDialog"
    :onclickSend="saveChanges"
  ...
  <dialogCreateObservation
    ...
    labelTextArea="Escriba esta observación para este Seguimiento"
...
async function loadDataFollowup() {
  ...
  if (!email) {
    notifyErrorRequest(
      "No se pudo obtener el correo del usuario. Por favor, verifica tu sesión."
    );
    loading.value = false;
    return;
  }
  ...
  try {
    if (idRegister) {
      const response = await getData(
        `/followup/listFollowupByRegister/${idRegister}`
      );
      console.log("seguimientos listados por asignación:", response);
      rows.value = response.followup;
    } else {
      const response = await getData(
        `/followup/listfollowupbyinstructoremail/${email}`
      );
    ...
    if (error.response && error.response.data && error.response.data.message) {
      ...
      messageError =
        error.response.data.errors[0].msg || "Error al cargar los seguimientos";
    } else {
...
async function handleSend() {
  ...
  try {
    const response = await putData(`/followup/addobservation/${id.value}`, {
...
async function searchApprentice() {
  try {
    ...
    const response = await getData(
      `/followup/listFollowupByRegister/${searchValue.value}`
    );
    ...
    if (searchValue.value === "") {
      ...
      if (error.response?.data?.message) {
        ...
        messageError =
          "No se encontró ningún aprendiz con la información seleccionada.";
      }
      notifyErrorRequest(messageError);
    }
  }
...
async function fetchApprenticeOptions() {
  try {
    ...
    notifyErrorRequest(
      "Error al cargar las opciones de búsqueda de aprendices."
    );
  }
}
